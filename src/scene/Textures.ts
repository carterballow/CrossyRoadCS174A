...
// Cached textures — created once, cloned for per-lane repeat settings
...
export function createGrassTexture(): THREE.CanvasTexture {
  if (!_grassTex) {
    _grassTex = createCanvasTexture(128, 128, (ctx) => {
...
export function createRoadTexture(): THREE.CanvasTexture {
  if (!_roadTex) {
    _roadTex = createCanvasTexture(256, 64, (ctx) => {
...
export function createRailwayTexture(): THREE.CanvasTexture {
  if (!_railwayTex) {
    _railwayTex = createCanvasTexture(128, 64, (ctx) => {
